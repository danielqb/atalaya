...
graph TD
    ...
    H --> O[ElevenLabs TTS]
    O --> P[Bot de voz Mumble]
    P --> Q[Clientes TAK conectados por voz]
```
...
- Webhook.
- API interna.
- Voz en canal Mumble: el aviso se sintetiza con ElevenLabs y un bot (`pymumble`) lo transmite en vivo a cualquier cliente TAK conectado por voz. Ver [mvp-spec.md](mvp-spec.md#7-reporte-por-audio-mumble) para el detalle de la integracion.
...
- ElevenLabs para texto-a-voz del aviso tactico.
- `pymumble` como cliente Python del bot de voz.
- Murmur (`ghcr.io/mumble-voip/mumble-server`) como servidor de voz, standalone via docker-compose (sin bridge ICE a cuentas OTS: ver mvp-spec.md).
