...
6. Un bot de voz retransmite el aviso por audio (ElevenLabs + Mumble) a cualquier cliente TAK conectado por voz.
...
- Login basico.
- Bot de voz (ElevenLabs TTS + `pymumble`) que retransmite el aviso tactico como audio en un canal Mumble.
...
Uso: texto-a-voz del aviso tactico ya generado por OpenAI/OpenRouter.

Justificacion: convierte el brief escrito en un reporte hablado que se retransmite en vivo a equipos conectados por voz, sin que ellos necesiten mirar una pantalla.

Funciones MVP:

- TTS del campo `brief` de cada `RiskAlert` con prioridad alta o critica.
- Voz en espanol, tono operativo/tactico.
- Fallback: si ElevenLabs falla, el aviso sigue disponible en texto en el dashboard (el audio es un canal adicional, no bloqueante).
...
graph TD
    ...
    G --> K[ElevenLabs TTS]
    K --> L[Bot Mumble]
    L --> M[Murmur server - docker-compose]
    M --> N[Clientes TAK conectados por voz]
```
...
### 7. Reporte Por Audio (Mumble)

Componentes:

- Servicio `murmur` (`ghcr.io/mumble-voip/mumble-server:latest`) agregado al `docker-compose.yml` de OpenTAKServer-Docker. Solo docker-compose para esta demo, no se toca el Helm chart ni el cluster.
- Bot Python (`src/audio/mumble_bot.py`, clase `TacticalVoiceBot`) que se conecta a Murmur con `pymumble`, sintetiza el `brief` de la alerta con ElevenLabs y transmite el PCM 48kHz mono al canal.

Decision tecnica importante: OTS trae un plugin `MumbleAuthenticator` para autenticar cuentas de voz contra las cuentas OTS via ICE, pero esa integracion esta rota en la version actual (host de conexion hardcodeado a `127.0.0.1:6502`, y `zeroc-ice` no esta empacado como dependencia). Reconstruir eso no es viable en 4h15. Se opto por Murmur **standalone**: el bot y cualquier cliente ATAK se conectan con credenciales propias de Murmur, sin pasar por las cuentas de OTS. El resultado observable de la demo es identico.

Disparo: el bot se invoca cuando el `CoordinatorAgent`/Alert Dispatcher emite una `RiskAlert` con prioridad `high` o `critical`.

```json
{
  "event_id": "demo-casevac-001",
  "brief": "Alpha-2 reporta un herido con sangrado moderado a 883 metros al este. Prioridad alta: coordinar extraccion medica y confirmar ruta segura.",
  "priority": "high"
}
```
...
| 12:00-14:00 (en paralelo) | Audio TAK | Murmur en docker-compose + bot ElevenLabs/pymumble, agente dedicado |
...
### Si el bot de voz Mumble falla

El audio es un canal adicional, no bloqueante: el aviso sigue visible en texto en el dashboard y en CoT hacia TAK. Mostrar el bot como demo aislada (`python -m src.audio.mumble_bot`) si la integracion en vivo con el pipeline no alcanza a cerrarse.
...
- Existe fallback si TAK, LLM, Auth0, Trigger.dev o el bot de voz fallan.
- El bot de voz habla el aviso de al menos una alerta de prioridad alta/critica en un cliente Mumble conectado.
...
5. Escuchar al bot retransmitir el aviso por voz en el canal Mumble.
6. Simular `HAZARD` desde Charlie-3.
7. Preguntar al copiloto: "Resume las alertas criticas activas".
8. Mostrar que el ruido `CHAT` no interrumpe el feed prioritario.
9. Cerrar con: "Atalaya convierte eventos TAK en instrucciones claras para equipos de emergencia en tiempo real, en texto y en voz".
